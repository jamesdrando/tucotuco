...
func TestParseExpr(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "substring string form", input: "SUBSTRING(source_text FROM 2)", want: "call(name(substring), [id(source_text), int(2)])"},
		{name: "substring string form with length", input: "SUBSTRING(source_text FROM 2 FOR 3)", want: "call(name(substring), [id(source_text), int(2), int(3)])"},
		{name: "trim from form", input: "TRIM(FROM source_text)", want: "call(name(trim), [id(source_text)])"},
		{name: "trim chars from form", input: "TRIM('.' FROM source_text)", want: "call(name(trim), [id(source_text), string(\".\")])"},
		{name: "trim leading form", input: "TRIM(LEADING '.' FROM source_text)", want: "call(name(ltrim), [id(source_text), string(\".\")])"},
		{name: "trim trailing from form", input: "TRIM(TRAILING FROM source_text)", want: "call(name(rtrim), [id(source_text)])"},
		{name: "position string form", input: "POSITION('x' IN source_text)", want: "call(name(position), [string(\"x\"), id(source_text)])"},
		{name: "overlay string form", input: "OVERLAY(source_text PLACING 'x' FROM 2)", want: "call(name(overlay), [id(source_text), string(\"x\"), int(2)])"},
		{name: "overlay string form with length", input: "OVERLAY(source_text PLACING 'x' FROM 2 FOR 3)", want: "call(name(overlay), [id(source_text), string(\"x\"), int(2), int(3)])"},
